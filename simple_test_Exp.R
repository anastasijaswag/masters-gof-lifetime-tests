# ==============================================================================
# simple_test_Exp.R
# LB-GOF критерий: простая гипотеза H0: F(t) = 1 - exp(-t/eta), eta известно
...
# ------------------------------------------------------------------------------
# Статистика Крамера–фон Мизеса для простой гипотезы (экспонента)
#
#   T*_m = 1/(12m) + sum_{i=1}^{m} [ U_{(i)} - (2i-1)/(2m) ]²
#
# где U_i = F0(X*_i) = 1 - exp(-X*_i / eta),  U_{(i)} — порядковые статистики
# ------------------------------------------------------------------------------

cvm_stat_simple_exp <- function(x_star, eta) {
  m <- length(x_star)
  u <- sort(pexp(x_star, rate = 1 / eta))
...
choose_m_simple_exp <- function(fit, n, eta,
                                alpha     = 0.05,
                                epsilon   = 0.02,
                                eta_param = 0.10,
                                gamma     = 1/3,
  ...
  x_max  <- qexp(1 - 1e-6, rate = 1 / eta)
  x_grid <- seq(0, x_max, length.out = 5000)

  F0_vals <- pexp(x_grid, rate = 1 / eta)
  f0_vals <- dexp(x_grid, rate = 1 / eta)
...
# ------------------------------------------------------------------------------
# Основная функция: LB-GOF критерий (простая гипотеза, экспонента)
#
#   H0 : F = Exp(eta)   (eta известно)
#
# Аргументы:
...
lb.gof.simple.exp <- function(L, R, eta,
  ...
  n <- length(L)
  stopifnot(length(R) == n, all(L <= R), eta > 0)

  # Шаг 1: NPMLE
  fit <- icfit(L, R)

  # Шаг 2: выбор m (ур. (16)–(18))
  m       <- choose_m_simple_exp(
    fit = fit, n = n, eta = eta,
  ...
  for (k in seq_len(n_pilot)) {
    ...
    T_pilot[k] <- cvm_stat_simple_exp(x_star, eta)
  }
  ...
  for (j in seq_len(N)) {
    x_star <- sample_from_npmle(fit, m)
    T_star <- cvm_stat_simple_exp(x_star, eta)
...
if (FALSE) {
  ...
  # H0 верна: данные из Exp(2), проверяем Exp(2)
  eta0 <- 2
  X    <- rexp(n, rate = 1 / eta0)
  Y    <- rexp(n, rate = 1 / 5)
  Z    <- Y + rexp(n, rate = 1)
